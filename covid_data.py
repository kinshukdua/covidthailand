...
from utils_scraping import CHECK_NEWER, MAX_DAYS, USE_CACHE_DATA, any_in, dav_files, get_next_number, get_next_numbers, \
    ...
    web_files, web_links, all_in, NUM_OR_DASH, s, logger
...
def scrape_and_combine():
    ...
    with Pool(processes=7) as pool:
        ...
        situation = situation.get(timeout=60*30)
        dashboard, dash_prov = dashboard__dash_prov.get(timeout=60*30)
        tests_reports = tests_reports.get(timeout=60*30)
        vac = vac.get(timeout=60*30)
        briefings_prov, cases_briefings = briefings_prov__cases_briefings.get(timeout=60*30)
        cases_demo, risks_prov = cases_demo__risks_prov.get(timeout=60*30)

        tweets_prov, twcases = tweets_prov__twcases.get(timeout=60*30)
        timelineapi = timelineapi.get(timeout=60*30)

        tests = tests.get(timeout=60*30)

        xcess_deaths.get(timeout=60*30)
        case_api_by_area = case_api_by_area.get(timeout=60*30)  # can be very wrong for the last days
